...
class TamperUI:
    ...
    def update_ui(self):
        ...
        try:
            ...
            if hasattr(self, 'voltage_value') and self.voltage_data:
                self.voltage_value.configure(text=f"{self.voltage_data[-1]:.2f} V")
                
            if hasattr(self, 'current_value') and self.current_data:
                self.current_value.configure(text=f"{self.current_data[-1]:.2f} A")
            ...
            if hasattr(self, 'magnetic_value') and self.magnetic_data:
                self.magnetic_value.configure(text=f"{self.magnetic_data[-1]:.2f} μT")
        ...
        try:
            if hasattr(self, 'axes') and self.axes:
                ...
                if 'magnetic' in self.axes:
                    ...
                    ax.clear()
                    ax.plot(self.magnetic_data, 'm-', label='Magnetic Field (μT)')
    ...
    def show_settings(self):
        ...
        com_entry.insert(0, self.reader.port if hasattr(self.reader, 'port') else "COM1")
        
        # Threshold settings
        threshold_frame = ctk.CTkFrame(settings_window)
        threshold_frame.pack(fill="x", padx=20, pady=10)
        
        ctk.CTkLabel(threshold_frame, text="Magnetic Threshold:").pack(side="left", padx=10)
        threshold_entry = ctk.CTkEntry(threshold_frame, width=100)
        threshold_entry.pack(side="left", padx=10)
        threshold_entry.insert(0, str(self.detector.magnetic_tamper_threshold if hasattr(self.detector, 'magnetic_tamper_threshold') else 50.0))
        
        # Save button
        save_btn = ctk.CTkButton(
            settings_window, 
            text="Save Settings",
            command=lambda: self.save_settings(com_entry.get(), float(threshold_entry.get()), settings_window)
        )
    ...
    def create_ui(self):
        ...
        header = ctk.CTkFrame(self.root, height=80, corner_radius=0, fg_color=COLORS["card"])
    ...
    def create_left_panel(self, parent):
        ...
        self.create_reading_card(parent, "Voltage", "0.0", "V", "voltage_label", COLORS["primary"])
        
        # Current reading
        self.create_reading_card(parent, "Current", "0.0", "A", "current_label", COLORS["success"])
        
        # Power reading
        self.create_reading_card(parent, "Power", "0.0", "W", "power_label", COLORS["warning"])
        
        # Magnetic Field reading
        self.create_reading_card(parent, "Magnetic Field", "0.0", "G", "magnetic_label", COLORS["secondary"])
        
        # Event Status Card with improved styling
        event_card = ctk.CTkFrame(parent, corner_radius=15, fg_color=COLORS["card"], border_width=1, border_color=COLORS["border"])
        ...
        self.settings_btn.pack(side="right", fill="x", expand=True, padx=(5, 0))
    ...
    def create_reading_card(self, parent, label, value, unit, attr_name, accent_color=None):
        ...
        card = ctk.CTkFrame(parent, corner_radius=15, fg_color=COLORS["card"], border_width=1, border_color=COLORS["border"])
    ...
    def create_graph(self, parent, y1_label, y2_label, data_attrs, colors):
        ...
        if y1_label:
            line1, = ax.plot([], [], color=colors[0], linewidth=2, label=y1_label)
        ...
        if y2_label and len(colors) > 1:
            ...
            line2, = ax2.plot([], [], color=colors[1], linewidth=2, label=y2_label)
        ...
        ax.set_title(f"Real-Time {y1_label} Monitoring", color='white', fontsize=12, fontweight='bold')
            ...
            for line, data_attr, axis in lines:
                ...
                if len(data) > 0:
                    ...
                    if len(data) > 1:
                        ...
                        y_max = max(data) * 1.1 if max(data) > 0 else max(data) * 0.9
                        if y_min == y_max:
                            y_max = y_min + 1
    ...
    def create_event_log_panel(self, parent):
        ...
        self.tree = ttk.Treeview(
            tree_frame,
            columns=("Time", "Voltage", "Current", "Power", "Mag Field", "Event", "Type"),
            show="headings",
    ...
    def create_stats_panel(self, parent):
        ...
        for i, category in enumerate(["Voltage (V)", "Current (A)", "Power (W)", "Magnetic (G)"]):
            ...
            cat_label.grid(row=i*2, column=0, columnspan=3, sticky="w", pady=(10, 5))
            
            # Stats row
            min_label = ctk.CTkLabel(stats_frame, text="Min:", text_color=COLORS["text_secondary"])
            min_label.grid(row=i*2+1, column=0, sticky="w", padx=(0, 10))
            
            avg_label = ctk.CTkLabel(stats_frame, text="Avg:", text_color=COLORS["text_secondary"])
            avg_label.grid(row=i*2+1, column=1, sticky="w", padx=10)
            
            max_label = ctk.CTkLabel(stats_frame, text="Max:", text_color=COLORS["text_secondary"])
        ...
        events_frame = ctk.CTkFrame(parent, fg_color=COLORS["card"], corner_radius=10)
        ...
        # Normal events
        normal_label = ctk.CTkLabel(events_grid, text="Normal:", text_color=COLORS["text_secondary"])
        ...
        self.normal_events_label.grid(row=0, column=1, sticky="e", padx=5, pady=5)
        
        # Tamper events
        tamper_label = ctk.CTkLabel(events_grid, text="Tamper:", text_color=COLORS["text_secondary"])
        ...
        self.tamper_events_label.grid(row=0, column=3, sticky="e", padx=5, pady=5)
        
        # Total readings
        total_label = ctk.CTkLabel(events_grid, text="Total Readings:", text_color=COLORS["text_secondary"])
        total_label.grid(row=1, column=0, columnspan=2, sticky="w", padx=5, pady=5)
        ...
        self.total_readings_label.grid(row=1, column=2, columnspan=2, sticky="e", padx=5, pady=5)
    ...
    def update_readings(self, data=None):
        ...
        try:
            ...
            if hasattr(self, 'magnetic_label'):
                self.magnetic_label.configure(text=f"{data['magnetic_field']:.2f}")
    ...
    def update_statistics(self):
        ...
        if logged_stats:
            if 'normal_events' in logged_stats and hasattr(self, 'normal_events_label'):
                self.normal_events_label.configure(text=str(logged_stats.get('normal_events', 0)))
            if 'tamper_events' in logged_stats and hasattr(self, 'tamper_events_label'):
                self.tamper_events_label.configure(text=str(logged_stats.get('tamper_events', 0)))
            if 'total_readings' in logged_stats and hasattr(self, 'total_readings_label'):
                self.total_readings_label.configure(text=str(logged_stats.get('total_readings', 0)))
        
        # Update event breakdown
        if logged_stats and 'event_types' in logged_stats and logged_stats['event_types']:
            breakdown_text = "\n".join([f"{k}: {v} events" for k, v in logged_stats['event_types'].items()])
